...
let PDFViewerApplication = {
  ...
  bindEvents() {
    ...
    eventBus.on('revertanonymization', webViewerRevertAnonymization);
  ...
  unbindEvents() {
    ...
    eventBus.off('revertanonymization', webViewerRevertAnonymization);
...
function webViewerRevertAnonymization() {
  let latestRectangle = null;
  let latestPage = null;

  for (let i = 0; i < PDFViewerApplication.anonymizationPages.length; i++) {
    const page = PDFViewerApplication.anonymizationPages[i];
    for (let j = 0; j < page.length; j++) {
      const rectangle = page[j];

      if (latestRectangle === null ||
        latestRectangle.createdAt < rectangle.createdAt) {
  ...
  if (latestRectangle && latestPage && latestPage.length) {
    latestPage.pop();

    let pageIndex = PDFViewerApplication.anonymizationPages.indexOf(latestPage);
    if (pageIndex >= 0 && pageIndex < PDFViewerApplication.pdfViewer._pages.length) {
      PDFViewerApplication.pdfViewer.renderAnnotationsAllPages();
    }
  }
}
